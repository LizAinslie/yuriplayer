...
fun WavySeekBar(
    ...
    // Only read `phase` (and therefore drive redraws) while the wave is visible,
    // so an idle/paused seek bar does not animate the Canvas every frame.
    ...
    Canvas(
        ...
        // Coarser step keeps the per-frame path cheap (debug builds stay usable).
        val step = 4f

        fun waveY(x: Float): Float {
            if (amp < 0.5f) return midY
            return midY + amp * sin(2f * PI.toFloat() * x / len + effectivePhase)
        }
