...
suspend fun expandAlbumTracksByName(
    ...
    if (name.isNotEmpty()) {
        ...
        if (bySongKey.size < 3) {
            ...
            if (needle.length >= 2) {
                ...
                AlbumLog.step(name, "short-name search", bySongKey.size, "hits=${hits.size} added=$added")
            }
        }
    }
    ...
    extraSeedSongs.take(4).forEach { seed ->
        ...
        dao.searchTracks(q.take(48), limit = 80).forEach { t ->
            ...
            val ak = t.albumKey
            if (!ak.isNullOrBlank()) {
                dao.getTracksForAlbum(ak).forEach { put(it) }
            }
        }
    }
...
private fun filenameTrackHint(song: Song): Int {
    val parsed = FilenameMetadataParser.parse(song.path ?: song.contentUri)
...
/** Canonical entity → Song mapping shared by all catalog code. */
fun CatalogTrackEntity.toSong(): Song = Song(
    id = id,
    title = title,
    artist = artist,
