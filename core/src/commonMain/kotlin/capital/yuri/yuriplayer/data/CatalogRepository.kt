...
/**
 * Shared, platform-agnostic catalog repository: local + remote-indexed tracks
 * (Jellyfin / Subsonic). Local scanning is injected via [localScanner] so
 * Android (MediaStore/SAF) and desktop (folder walk) both satisfy it.
 */
class CatalogRepository(
    private val dao: CatalogDao,
    private val sourceInstances: SourceInstanceRepository,
    private val localScanner: LocalScanner,
    private val coverCandidateBuilder: CoverCandidateBuilder = CoverCandidateBuilder { songs, path, url ->
        // Default: no local filesystem probing; Android/JVM hosts inject their own.
        PureCoverCandidates.build(songs, path, url)
    }
) {
    ...
    suspend fun coverCandidatesForAlbum(albumKey: String, tracks: List<Song> = emptyList()): List<CoverCandidate> =
        withContext(Dispatchers.IO) {
            ...
            val row = dao.getAlbum(albumKey)
            coverCandidateBuilder.build(songs, row?.coverPath, row?.coverUrl)
        }
    ...
    private suspend fun ingestRemoteDeltaLocked(
        ...
        val incoming = songs.map { song ->
            song.toTrackEntity(
                ...
                externalId = song.path ?: song.contentUri,
                seenAt = seenAt
            )
        }
    ...
    suspend fun applyAlbumYear(albumKey: String, year: Int) = withContext(Dispatchers.IO) {
        ...
        val tracks = expandAlbumTracksLocked(albumKey).mapNotNull { song ->
            dao.getTrack(song.songKey)?.let { t ->
                val ty = t.year
                if (ty == null || ty <= 0) t.copy(year = year) else t
            }
        }
    ...
    private fun CatalogTrackEntity.sameRemoteMeta(incoming: CatalogTrackEntity): Boolean {
        ...
        val incDur = incoming.durationMs
        if (incDur != null && incDur > 0 && durationMs != incDur) return false
