 ...
 * ```
 * adb shell setprop log.tag.YuriPlayer.YuriAlbum VERBOSE
 * adb logcat -v time YuriPlayer.YuriAlbum:V *:S
 * adb logcat -v time YuriPlayer.YuriAlbum:V *:S | grep -i clancy
 * adb logcat -v time YuriPlayer.YuriAlbum:V *:S | grep -i 'twenty one pilots'
 * ```
 *
 * Android Studio Logcat: package `capital.yuri.yuriplayer`, tag `YuriPlayer.YuriAlbum`,
 * level **Verbose**.
 *
 * Levels:
