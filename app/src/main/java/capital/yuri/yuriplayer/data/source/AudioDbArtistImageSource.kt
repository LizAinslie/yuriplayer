package capital.yuri.yuriplayer.data.source

import android.util.Log
import io.ktor.client.HttpClient
import io.ktor.client.request.get
import io.ktor.client.statement.bodyAsText
import io.ktor.http.isSuccess
import kotlinx.coroutines.Dispatchers
import kotlinx.coroutines.withContext
import org.json.JSONObject
import java.net.URLEncoder

/**
 * TheAudioDB free API (test key "2").
 * Good for profile thumbs + wide fanart/banners.
 */
class AudioDbArtistImageSource(
    private val http: HttpClient
) : ArtistInfoSource {

    override val id: String = "theaudiodb"
    override val displayName: String = "TheAudioDB"

    override suspend fun fetchProfile(artistName: String): ArtistProfile? = null

    override suspend fun fetchImageCandidates(
        artistName: String,
        kind: ArtistImageKind
    ): List<ArtistImageCandidate> = withContext(Dispatchers.IO) {
        val q = URLEncoder.encode(artistName.trim(), "UTF-8")
        val body = get("https://www.theaudiodb.com/api/v1/json/$API_KEY/search.php?s=$q")
            ?: return@withContext emptyList()
        try {
            val artists = JSONObject(body).optJSONArray("artists") ?: return@withContext emptyList()
            val out = LinkedHashMap<String, ArtistImageCandidate>()
            for (i in 0 until artists.length()) {
                val a = artists.optJSONObject(i) ?: continue
                val name = a.optString("strArtist").ifBlank { artistName }
                when (kind) {
                    ArtistImageKind.PROFILE -> {
                        listOf(
                            "strArtistThumb" to "AudioDB thumb",
                            "strArtistWideThumb" to "AudioDB wide thumb",
                            "strArtistClearart" to "AudioDB clearart",
                            "strArtistCutout" to "AudioDB cutout"
                        ).forEach { (field, label) ->
                            val url = a.optString(field).takeIf { it.isNotBlank() && it != "null" }
                                ?: return@forEach
                            if (url !in out) out[url] = ArtistImageCandidate(url, id, "$label · $name")
                        }
                    }
                    ArtistImageKind.BANNER -> {
                        listOf(
                            "strArtistFanart" to "AudioDB fanart",
                            "strArtistFanart2" to "AudioDB fanart 2",
                            "strArtistFanart3" to "AudioDB fanart 3",
                            "strArtistBanner" to "AudioDB banner"
                        ).forEach { (field, label) ->
                            val url = a.optString(field).takeIf { it.isNotBlank() && it != "null" }
                                ?: return@forEach
                            if (url !in out) out[url] = ArtistImageCandidate(url, id, "$label · $name")
                        }
                    }
                }
            }
            out.values.toList()
        } catch (e: Exception) {
            Log.w(TAG, "parse failed", e)
            emptyList()
        }
    }

    private suspend fun get(url: String): String? {
        return try {
            val response = http.get(url)
            if (!response.status.isSuccess()) null else response.bodyAsText()
        } catch (e: Exception) {
            Log.w(TAG, "GET $url", e)
            null
        }
    }

    companion object {
        private const val TAG = "AudioDbArtistImg"
        /** Public demo key documented by TheAudioDB. */
        private const val API_KEY = "2"
    }
}
